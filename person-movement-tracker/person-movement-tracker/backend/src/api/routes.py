...
def _resolve_video_input(video_input: Any) -> Path:
    if isinstance(video_input, int):
        video_record = get_video(video_input)
        if not video_record:
    ...
    if isinstance(video_input, str) and video_input.isdigit():
        video_record = get_video(int(video_input))
        if not video_record:
            raise HTTPException(status_code=404, detail=f"Video not found: {video_input}")
        return _video_record_path(video_record)
...
async def _process_recorded_video(
    ...
    filename = f"session_{session_id}_{exercise_type}_{timestamp}.mp4"
    ...
    if video_analysis_service:
        try:
            ...
            update_video_analysis(video_id, "completed")
        except Exception as e:
        ...
        try:
            ...
            if reference_video_id:
                ...
                if not reference_record:
                    comparison_error = f"Reference video not found: {reference_video_id}"
                else:
    ...
    try:
        while True:
            ...
            if "config" in data:
                ...
                exercise_type = config_data.get("exercise_type", "squat")
                duration_seconds = config_data.get("duration_seconds", 15)
                reference_video_id = config_data.get("reference_video_id")
