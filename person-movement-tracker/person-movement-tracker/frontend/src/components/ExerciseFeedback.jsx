...
function ExerciseFeedback({ analysis }) {
  ...
  const getAngleNotes = () => {
    ...
    const angleKeys = safeAngles && Object.keys(safeAngles).length > 0 ? Object.keys(safeAngles) : [];
    if (!angleKeys.length) return [];

    const notes = [];
    const leftKnee = safeAngles.leftKnee ?? safeAngles.left_knee;
    const rightKnee = safeAngles.rightKnee ?? safeAngles.right_knee;
    const leftHip = safeAngles.leftHip ?? safeAngles.left_hip;
    const rightHip = safeAngles.rightHip ?? safeAngles.right_hip;
